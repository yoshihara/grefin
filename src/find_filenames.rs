...
pub fn find_files(config: &Config) -> Result<(), String> {
    ...
    find_files_in_directory(&mut stdout, &config.query, &config.path, ".git")
}

fn find_files_in_directory<W: Write>(w: &mut W, query: &str, dir: &str, ignored_query: &str) -> Result<(), String>{
        ...
        if let Ok(entry) = entry {
            ...
            if let Some(filename) = path.file_name() {
                ...
                if filename_str == ignored_query {
                    ...
                    if let Err(err) = find_files_in_directory(w, query, path.to_str().ok_or("converting path error".to_string())?, ignored_query) {
                        return Err(err);
                    }
                } else {
                    if filename_str.to_ascii_lowercase().contains(query) {
...
mod test_find_files_in_directory {
    ...
    fn test_no_hit_file_in_git() {
        ...
        assert!(find_files_in_directory(&mut output_stub, "this_is_fixture_file", "fixtures/", "git").is_ok());
